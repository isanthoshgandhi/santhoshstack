...
| `usability-heuristics` | Deterministic usability audit against all 10 Nielsen-Norman heuristics. Works on UI screens, CLI flows, API patterns, component code, or any described interface. Every run covers the same 10 heuristics in the same order with the same severity scale (CRITICAL/HIGH/MEDIUM/LOW). Produces a structured report: heuristic scan, severity-sorted findings with location and fix, and a top-3 highest-impact action list. | `/usability-heuristics` |
